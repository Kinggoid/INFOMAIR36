...
def suggest_restaurant(available_restaurants, additional_preferences):
...
def apply_inference_rules(restaurant_df, user_input):
    ...
    valid_restaurants_df = pd.DataFrame(columns=list(restaurant_df.columns) + ['inf1', 'inf2', 'inf3', 'inf4', 'inf5', 'inf6'])
    ...
    for _, row in restaurant_df.iterrows():
        ...
        # Inf1
        if price == "cheap" and "food_quality" == "good":
            inf1 = "touristic"
        else:
            inf1 = "touristic"
        ...
        if crowdedness == "busy":
            ...
            inf3 = "not assigned seats"

        # Inf4
        if stay_duration != "long stay":
            inf4 = "children"
        else:
            inf4 = "not children"
        ...
        valid_restaurants_df = pd.concat([valid_restaurants_df, pd.DataFrame([new_row])], ignore_index=True)

    # Reset the index of the new DataFrame
    valid_restaurants_df.reset_index(drop=True, inplace=True)
    print(valid_restaurants_df[['restaurantname', 'inf1', 'inf2', 'inf3', 'inf4', 'inf5', 'inf6']])
